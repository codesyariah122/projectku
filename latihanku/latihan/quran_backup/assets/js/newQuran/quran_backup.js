const NullOptionsSurah = document.createElement('option')
...
const NullOptionsAyat = document.createElement('option')
...
document.querySelector('#enter').addEventListener('click', function () {
	...
	const NumberAyat = document.querySelector('#select-ayat').value
	...
	if(NumberSurah == 'choose' || NumberSurah == null ){
		...
		ViewSurah(QuranApi.url, NumberSurah)
		.then(res => {
			...
			setTimeout(() => {
				...
				document.querySelector('.card-header').appendChild(headerEl, bodyEl)
				document.querySelector('#loader').style.visibility="hidden"
			}, 1500)
...
const ViewAyat = (data, id) => {
	...
	ReadAyat(QuranApi.url,QuranApi.req, NumberSurah, NumberAyat, res => {
	...
	}, (err) => console.log(`Error results : ${err}`))
}
...
const NextAyat = (surah, ayat) => {
	...
	ReadAyat(QuranApi.url, QuranApi.req, DataSurah, next, res => {
...
const LastAyat = (surah, ayat) => {
	...
	const last = ayat
		...
		ReadAyat(QuranApi.url, QuranApi.req, DataSurah, last, res => {
...
const PrevAyat = (surah, ayat) => {
	...
	ReadAyat(QuranApi.url, QuranApi.req, DataSurah, prev, res => {
